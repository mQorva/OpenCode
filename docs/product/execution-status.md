...
| AP-11 | in-progress | Protected Windows secret storage, paste-key and PKCE flows, safe account/model projections, local API, generated SDK, engine credential projection, and first-class settings-v2 account UI pass focused checks and Desktop production build. Detailed model selection, remaining metadata states, and one explicitly approved live verification remain. |
