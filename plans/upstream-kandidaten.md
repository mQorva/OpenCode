...
| 7 | `permission-dock-layout` | #32 | **veröffentlicht 11.09.2026** als PR #48536 (`permission-dock-layout`, Issue #48532). Opt-in `footerInside` in `dock-prompt.tsx` (Upstream hatte `DockTray` schlanker gezogen, `footerInside` nie eingeführt), Story mit `footerInside: true`, Permission-Footer-CSS (flex-wrap). `tsgo` grün in `packages/app` + `session-ui`. UI-Nachweis (Screenshot) noch offen | auf Maintainer-Feedback warten |
...
Damit sind 10 PR-Pakete (alle 10 veröffentlicht; `followup-queue-controls` entfällt) der „Non-v2"-Strecke definiert. Die Pakete
0–6, 7, 8 und 9 sind veröffentlicht (Paket 7 `permission-dock-layout` seit 11.09.2026).
...
## Veröffentlichte / vorgemerkte Pakete (Status 11.09.2026)

Die folgenden Pakete sind entweder als PR veröffentlicht oder als `vorgemerkt` im Inventar erfasst. Die vier am 11.09.2026 gesendeten PRs (`prompt-submit-fixes`, `session-error-sync`, `workspace-terminals`, `permission-dock-layout`) wurden aus dem aktuellen `dev`-Diff gegen `upstream/dev` (`95daf90670`) als isolierte Worktree-Branches extrahiert. Details bleiben in `plans/upstream/` oder als Worktree lokal.

| Paket | Kandidat(en) | Dateien (Haupt) | Status | Nächster Schritt |
|---|---|---|---|---|
| `prompt-submit-fixes` | #15 + #16 | `session-ui/.../prompt-input/editor-dom.ts` (neu) + Export, `app/.../prompt-input/submit.ts`, `server-compat.ts`, `submit.test.ts` | **veröffentlicht 11.09.** PR #48533 (`prompt-submit-fixes`), Issues #48528, #48529; 3 Commit-Serie; `submit.test.ts` 9/9; `tsgo` grün | auf Maintainer-Feedback warten |
| `session-error-sync` | #12 | `app/.../global-sync/event-reducer.ts`, `server-sync.tsx`, `event-reducer.test.ts` | **veröffentlicht 11.09.** PR #48534 (`session-error-sync`), Issue #48530; `event-reducer.test.ts` 17/17; `tsgo` grün | auf Maintainer-Feedback warten |
| `workspace-terminals` | #23 | `app/.../context/terminal.tsx`, `app.tsx`, `terminal.test.ts` | **veröffentlicht 11.09.** PR #48535 (`workspace-terminals`), Issue #48531; App-Level-`TerminalRegistryProvider`; `terminal.test.ts` 8/8; `tsgo` grün | auf Maintainer-Feedback warten |
| `permission-dock-layout` | #7 (#32) | `session-ui/.../dock-prompt.tsx`, `dock-prompt.stories.tsx`, `message-part.css`, `app/.../session-permission-dock.tsx` | **veröffentlicht 11.09.** PR #48536 (`permission-dock-layout`), Issue #48532; opt-in `footerInside`, kein Persistent/ButtonV2; `tsgo` grün; **UI-Nachweis (Screenshot) noch offen** | auf Maintainer-Feedback warten; ggf. Screenshot nachreichen |
...
| `app-v1` | #13, #14, #17 | `context/global-sync/bootstrap.ts`, `context/server-sdk.tsx`, `utils/server-errors.ts` | **v2-blockiert**: alter App-Pfad (`v1`) fehlt in `v2`; erst nach `v2`-Übernahme neu reproduzieren und extrahieren. (#12, #15, #16, #23 sind als `session-error-sync`/`prompt-submit-fixes`/`workspace-terminals` bereits veröffentlicht) | Architektur-Gate `v2` abwarten |
...
| `thinking-heading-truncate` | #41 | `session-ui/src/components/session-turn.css`, `session-turn.tsx`, `pages/session/timeline/message-timeline.tsx` | **bereits veröffentlicht** als PR #48008 (`thinking-heading-truncate`, 08.09.); Inventar-Eintrag fehlte nur | auf Maintainer-Feedback warten |
